...
import { _getJupiterPrices } from "./jupiter-token-list-api";
import RaydiumDownloader from "./clmm-downloader";
...
export default class ClmmDb {
    _db;
    _addInstructionStatement;
    _addTransferStatement;
    _addPairStatement;
    _addTokenStatement;
    _addUsdYStatement;
    _addUsdXStatement;
    _fillMissingUsdStatement;
    _setOldestSignature;
    _markCompleteStatement;
    _getAllTransactions;
    _downloaders = new Map();
    _saving = false;
    _queue = [];
    delaySave = false;
    constructor() {
    ...
    async addInstruction(instruction) {
        await this._queueDbCall(() => {
            const { signature: $signature, slot: $slot, blockTime: $block_time, instructionName: $instruction_name, instructionType: $instruction_type, accounts, activeBinId: $active_bin_id = null, // ← comes from the interface now
            removalBps: $removal_bps = null, } = instruction;
            const { position: $position_address, pool: $pair_address, sender: $owner_address, } = accounts;
    ...
    async getPoolForPosition(position_address) {
    ...
    async addPair(pair) {
        await this._queueDbCall(() => {
            const { lbPair: $pair_address, name: $name, mintX: $mint_x, mintY: $mint_y, tickSpacing: $tick_spacing, feeBps: $fee_bps, } = pair;
    ...
    async getUsdTransactions(position_address) {
    ...
    `)[0]?.values ?? [];
        ...
        const mints = [...new Set(rows.map(r => r[3]))];
        let prices = {};
        // Raydium price endpoint – returns { data: { mint: price } }
        try {
            const ray = await fetch('https://api-v3.raydium.io/mint/price?ids=' + mints.join(',')).then(r => r.json());
            prices = ray?.data ?? {};
        }
        ...
        for (const [signature, slot, blockTime, mint, rawAmount, decimals] of rows) {
            ...
            const usd = amount * (prices[mint] ?? 0);
            if (!map.has(key)) {
                map.set(key, {
                    signature: key,
                    slot: Number(slot),
    ...
    async addUsdTransactions(position_address, usdTxs) {
        await this._queueDbCall(() => {
    ...
    async getOwnerTransactions(owner_address) {
        return this._queueDbCall(() => {
            const result = this._db.exec(`SELECT *
                 FROM v_transactions
                 where owner_address = '${owner_address}'`);
...
//# sourceMappingURL=clmm-db.js.map