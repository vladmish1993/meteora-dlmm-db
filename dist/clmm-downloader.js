...
export default class RaydiumDownloader {
    ...
    _fetchingMissingPairs = false;
    _fetchingMissingTokens = false;
    _fetchingUsd = false;
    _onDone;
    _isDone = false;
    _finished = false;
    _startTime;
    _accountSignatureCount = 0;
    _positionTransactionIds = new Set();
    _positionAddresses = new Set();
    _usdPositionAddresses = new Set();
    _isComplete = false;
    _transactionDownloadCancelled = false;
    _fullyCancelled = false;
    _oldestSignature = "";
    _oldestBlocktime = 0;
    ...
    async _init(config) {
        if (config.account.length >= 43 && config.account.length <= 44) {
            ...
            const instructions = parseRaydiumInstructions(parsedTransaction);
            if (instructions.length == 0) {
                throw new Error(`${config.account} is not a Raydium CLMM transaction`);
            }
    ...
    async _loadInstructions(transactions) {
        ...
        transactions.forEach((transaction) => {
            ...
            parseRaydiumInstructions(transaction).forEach(async (instruction) => {
                ...
                instructionCount++;
                if (instruction.accounts.pool == "") {
                    await this._addMissingPool(instruction);
                }
    ...
    async _addMissingPool(instruction) {
        while (instruction.accounts.pool == "") {
            const pool = await this._db.getPoolForPosition(instruction.accounts.position);
            if (pool) {
                instruction.accounts.pool = pool;
            }
            else {
                await delay(1);
            }
    ...
    async _fetchMissingPairs() {
        ...
        let missingPairs = await this._db.getMissingPools();
        if (missingPairs.length > 0) {
            ...
            while (missingPairs.length > 0) {
                ...
                if (address) {
                    ...
                    missingPairs = await this._db.getMissingPools();
                }
            }
    ...
    async _getMissingToken(address) {
        ...
        const tokenData = await this._connection.getParsedAccountInfo(new PublicKey(address));
        if (tokenData.value &&
...
//# sourceMappingURL=clmm-downloader.js.map